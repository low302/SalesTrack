...
function StatCard({ title, value, subtitle, icon: Icon, trend, color = 'secondary' }) {
    return (
...
function ProgressCard({ title, current, target, color = 'secondary' }) {
...
function PaceCard({ soldUnits, color = 'secondary' }) {
    const now = new Date();
...
export default function UsedCarDashboardPage() {
    const [data, setData] = useState(null);
    const [loading, setLoading] = useState(true);
    const [soldListOpen, setSoldListOpen] = useState(false);
    const { getUsedDashboard, getSalespeople } = useApi();
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-base-content/60">Used vehicle sales overview and analytics</p>
            </div>

            {/* Key Metrics */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard
                    title="Today's Used Sales"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <PaceCard soldUnits={data.month.count} color="secondary" />
                <ProgressCard
                ...
                >
                    <div className="flex items-center justify-between">
                        <h2 className="card-title text-lg">
                            Used Car Sold List (This Month)
                            <span className="badge badge-secondary">{data.soldList?.length || 0}</span>
                        </h2>
                        {soldListOpen ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="card bg-base-100 shadow-md">
                    <div className="card-body">
                        <h2 className="card-title text-lg">Daily Used Car Sales (Last 30 Days)</h2>
                        <div className="h-72">
            ...
            <div className="card bg-base-100 shadow-md">
                <div className="card-body">
                    <h2 className="card-title text-lg">Used Car Sales Leaderboard (This Month)</h2>
                    <div className="overflow-x-auto">
                        <table className="table">
                            ...
                            <tbody>
                                {data.salesBySalesperson.filter(sp => sp.salesCount > 0).map((sp, index) => (
                                    <tr key={sp.id} className={index === 0 ? 'bg-secondary/5' : ''}>
                                        <td>
                                            <span className={`badge ${index === 0 ? 'badge-secondary' :
                                                index === 1 ? 'badge-accent' :
                                                    index === 2 ? 'badge-warning' :
                                                        'badge-ghost'
                                                }`}>
                                                #{index + 1}
                                            </span>
