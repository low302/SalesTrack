...
export default function TeamTracker() {
    ...
    return (
        <div className="h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 text-white p-6 overflow-hidden flex flex-col">
            {/* Header */}
            <div className="relative mb-6 flex-shrink-0">
                ...
                <div className="text-center">
                    ...
                    {settings?.quoteOfTheDay && (
                        <p className="text-xl text-blue-400 font-medium italic max-w-4xl mx-auto">
                            "{settings.quoteOfTheDay}"
                        </p>
                    )}
                </div>
            ...
            <div className="grid grid-cols-4 gap-4 mb-6 flex-shrink-0">
                <div className="bg-gray-800/50 backdrop-blur rounded-2xl p-5 border border-gray-700/50">
                    <div className="text-gray-400 text-base font-medium mb-2">New Sold Today</div>
                    <div className="text-5xl font-bold text-white">
                        {todaySales.todayNewCount % 1 === 0 ? todaySales.todayNewCount : todaySales.todayNewCount.toFixed(1)}
                    </div>
                </div>
                <div className="bg-gray-800/50 backdrop-blur rounded-2xl p-5 border border-gray-700/50">
                    <div className="text-gray-400 text-base font-medium mb-2">Used Sold Today</div>
                    <div className="text-5xl font-bold text-white">
                        {todaySales.todayUsedCount % 1 === 0 ? todaySales.todayUsedCount : todaySales.todayUsedCount.toFixed(1)}
                    </div>
                </div>
                <div className="bg-gray-800/50 backdrop-blur rounded-2xl p-5 border border-gray-700/50">
                    <div className="text-gray-400 text-base font-medium mb-2">Total New Car Gross</div>
                    <div className={`text-5xl font-bold ${competitionStats.totalNewGross >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                        {formatCurrency(competitionStats.totalNewGross)}
                    </div>
                </div>
                <div className="bg-gray-800/50 backdrop-blur rounded-2xl p-5 border border-gray-700/50">
                    <div className="text-gray-400 text-base font-medium mb-2">Total Used Car Gross</div>
                    <div className={`text-5xl font-bold ${competitionStats.totalUsedGross >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            ...
            <div className={`grid gap-4 flex-1 overflow-hidden ${sortedTeams.length === 2 ? 'grid-cols-2' : sortedTeams.length >= 3 ? 'grid-cols-3' : 'grid-cols-1'}`}>
                {sortedTeams.map((team, index) => {
                    ...
                    return (
                        <div
                            key={team.id}
                            className="bg-gray-800/60 backdrop-blur rounded-2xl overflow-hidden border border-gray-700/50 transition-all duration-300 flex flex-col"
                            ...
                            <div className="p-4 border-b border-gray-700/50 flex-shrink-0">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex-1 min-w-0 flex flex-col items-center text-center">
                                        <div className="flex items-center justify-center gap-2">
                                            <h2 className="text-3xl font-bold text-white truncate">{team.name}</h2>
                                            {team.contestName && (
                                                <span
                                    ...
                                    <div className="flex items-center gap-2 flex-shrink-0 bg-gray-900/50 rounded-xl px-4 py-3">
                                        <div className="text-center px-3 border-r border-gray-700">
                                            ...
                                            <div className="text-gray-400 text-xs font-medium mt-0.5">New</div>
                                        </div>
                                        <div className="text-center px-3 border-r border-gray-700">
                                            <div className="text-3xl font-bold text-orange-400 tabular-nums">
                                                {totalUsedCount % 1 === 0 ? totalUsedCount : totalUsedCount.toFixed(1)}
                                            </div>
                                            <div className="text-gray-400 text-xs font-medium mt-0.5">Used</div>
                                        </div>
                                        <div className="text-center px-3">
                                            <div className="text-4xl font-bold text-white tabular-nums">
                                                {totalCount % 1 === 0 ? totalCount : totalCount.toFixed(1)}
                                            </div>
                                            <div className="text-gray-400 text-xs font-medium mt-0.5">Total</div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-4 mt-3">
                                    <div className="flex-1 text-center">
                                        <div className={`text-xl font-bold ${totalNewGross >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                            {formatCurrency(totalNewGross)}
                                        </div>
                                        <div className="text-gray-500 text-xs mt-0.5">New Car Gross</div>
                                    </div>
                                    <div className="flex-1 text-center">
                                        <div className={`text-xl font-bold ${totalUsedGross >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                            {formatCurrency(totalUsedGross)}
                                        </div>
                                        <div className="text-gray-500 text-xs mt-0.5">Used Car Gross</div>
                                    </div>
                                    {team.goal > 0 && (
                                        <div className="flex-1">
                                            <div className="flex items-center justify-center gap-1 mb-1">
                                                <Target size={14} className="text-gray-400" />
                                                <span className="text-gray-400 text-sm">{team.goal}</span>
                                            </div>
                            ...
                            <div className="p-4 flex-1 overflow-y-auto">
                                <div className="space-y-3">
                                    {memberStats.slice(0, 10).map((member, memberIndex) => (
                                        ...
                                        >
                                            {/* Rank */}
                                            <div className="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center text-base font-bold text-gray-300 flex-shrink-0">
                                                {memberIndex + 1}
                                            </div>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="text-xl font-semibold text-white truncate">{member.name}</div>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-3 flex-shrink-0">
                                                ...
                                                <div className="flex items-center gap-2 bg-gray-800/50 rounded-lg px-4 py-2">
                                                    ...
                                                    <div className="text-center w-12 border-l border-gray-600 pl-2">
                                                ...
                                                <div className="text-center w-24">
                                                    <div className={`text-base font-semibold ${member.frontEnd >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                                        {formatCurrency(member.frontEnd)}
                                                    </div>
                                                    <div className="text-[10px] text-gray-500 mt-0.5">Front</div>
                                                </div>
                                                <div className="text-center w-24">
                                                    <div className={`text-base font-semibold ${member.backEnd >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                                        {formatCurrency(member.backEnd)}
                                                    </div>
                                                    <div className="text-[10px] text-gray-500 mt-0.5">Back</div>
                                                </div>
                                                <div className="text-center w-24">
                                                    <div className={`text-base font-semibold ${member.gross >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                    ...
                                    {memberStats.length === 0 && (
                                        <div className="text-center text-gray-500 py-4 text-lg">No members assigned</div>
                                    )}
                                </div>
                            </div>
                        </div>
            ...
            {/* Footer */}
            <div className="mt-4 text-center text-gray-600 text-sm flex-shrink-0">
